...
public class PrivateRoomWrapper {
    ...
    public String wrap(String user, String hostUsername){
        return hostUsername.equals(user) 
                ...
                "<a href='#'>" + user + "</a>" 
                : 
                String.format("<a target='_blank' onclick='sendNotification(this)' href='http://localhost:8080/WebSocketChat/FrontController?username=%s&room=%s&command=Anonymous&isPrivate=1'>%s</a>", hostUsername.substring(1, hostUsername.length() - 1), generateHash(), user);
    }
